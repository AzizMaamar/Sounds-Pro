...
const ProductScreen = ({ history, match }) => {
	...
	const compareWithOtherProducts = () => {
		history.push(`/productss/compare`);
	};
	const changeColor = async()=>{
		const path = "http://localhost:5000/api/products/color";
		const res = await axios.post(path,productColor,{withCredentials: false});
		setproductColor({})
					...
					<Row>
						...
						<Col md={3}>
							<ListGroup variant='flush'>
								...
								<Button
											 onClick={()=>{setproductColor(greenData)}} 
											style={{backgroundColor:"blue", width:'50px',height:'50px', borderRadius:"50%"}}
									/>
									
								<Button
											 onClick={()=>{setproductColor(blueData)}} 
											style={{backgroundColor:"red", width:'50px',height:'50px', borderRadius:"50%"}}
									/>
									
								<Button
											 onClick={()=>{setproductColor(yellowData)}} 
											style={{backgroundColor:"green", width:'50px',height:'50px', borderRadius:"50%"}}
									/>
									</div>
									<Button
											 onClick={()=>{changeColor();timeout(2000);setOpen(true);}} 
											className='btn-block'
											type='button'
										>
								...
								<ListGroup.Item>
								<Button
											onClick={compareWithOtherProducts}
											className='btn-block'
											type='button'
